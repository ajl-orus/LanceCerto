...
class ControladorLibroDiarioTests(unittest.TestCase):
    def setUp(self):
        licitante = Usuarios.crear(C.ID_USUARIO, C.NOMBRE_USUARIO, C.APELLIDO_USUARIO, C.EMAIL_USUARIO, C.NOMBRE_USUARIO,
                                 C.CLAVE_USUARIO, C.FECHA_NACIMIENTO_USUARIO, TipoDeUsuario.Licitante)
        ...
        usuarios = UsuariosEnMemoria(self._list_to_dictionary([licitante, consignatario]))
        ...
        self.__db = CreadorDeBasesDeDatosTemporales() \
            ...
            .con_pujas(PujasEnMemoria([Puja(C.PUJA_UID, licitante, lote, C.MONTO_PUJA)])) \
            .construir()
    ...
    def _list_to_dictionary(self, usuarios: list[Usuario]):
        ...
        for usuario in usuarios:
            diccionario[usuario.obtener_usuario()] = {
                ...
                "nome": usuario.obtener_nombre(),
                "sobrenome": usuario.obtener_apellido(),
                "email": usuario.obtener_email(),
                "usuario": usuario.obtener_usuario(),
                "clave": usuario.obtener_clave(),
                "nascimento": usuario.obtener_nacimiento(),
    ...
    @data(None, "", -1, 0)
    def test_retornar_error_listando_compras_de_licitante_invalido(self, licitante_invalido):
        sut = ControladorLibroDiario(self.__db)
        sut.listar_compras_de(licitante_invalido)
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual(ServicioLibroDiario.LICITANTE_INVALIDO, respuesta["mensaje"])

    def test_retornar_error_listando_compras_de_licitante_inexistente(self):
        sut = ControladorLibroDiario(self.__db)
        sut.listar_compras_de(C.OTRO_ID_USUARIO)
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual(ServicioLibroDiario.LICITANTE_INEXISTENTE, respuesta["mensaje"])

    def test_retornar_lista_vacia_con_licitante_sin_compra(self):
        sut = ControladorLibroDiario(self.__db)
    ...
    def test_retornar_lista_con_compras_correctamente(self):
        ...
        respuesta = sut.obtener_respuesta()
        self.assertEqual("ok", respuesta["status"])
        self.assertEqual([{"id": 1, "titulo": "Sofa Antigo", "ganhador": "Roberto", "preço": 616.0, "comissão": 50.0,
    ...
    def test_retornar_error_al_cerrar_lote_invalido(self, lote_invalido):
        ...
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual("Não pode fechar um lote não válido", respuesta["mensaje"])
    ...
    def test_retornar_error_al_cerrar_lote_inexistente(self):
        ...
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual("Não pode fechar um lote inexistente", respuesta["mensaje"])
    ...
    def test_retornar_ok_al_cerrar_lote_sin_pujas(self):
        ...
        respuesta = sut.obtener_respuesta()
        self.assertEqual("ok", respuesta["status"])
        self.assertEqual("Lote vazio", respuesta["mensaje"])
