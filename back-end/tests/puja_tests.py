...
class ControladorPujaTests(unittest.TestCase):
    ...
    @data("", None, -1, 0)
    def test_retornar_error_al_agregar_puja_con_licitante_invalido(self, licitante_invalido):
        sut = ControladorPuja(self.__db)
        sut.agregar(C.LOTE_UID, licitante_invalido, C.MONTO_PUJA)
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual(ServicioPuja.PUJAR_SIN_LICITANTE, respuesta["mensaje"])
    ...
    def test_retornar_error_al_agregar_puja_con_licitante_inexistente(self):
        sut = ControladorPuja(self.__db)
        sut.agregar(C.LOTE_UID, C.OTRO_ID_USUARIO, C.MONTO_PUJA)
        respuesta = sut.obtener_respuesta()
        self.assertEqual("error", respuesta["status"])
        self.assertEqual(ServicioPuja.LICITANTE_INEXISTENTE, respuesta["mensaje"])

    @data(TipoDeUsuario.Consignatario, TipoDeUsuario.Martillero)
    def test_retornar_error_al_agregar_puja_con_usuario_no_licitante(self, tipo: TipoDeUsuario):
        sut = ControladorPuja(self.__db)
    ...
    def test_agregar_puja_correctamente(self):
        ...
        self.assertEqual(C.ID_USUARIO, puja.obtener_licitante().obtener_uid())
    ...
    def test_retornar_lista_cuando_hay_pujas(self):
        ...
        respuesta = sut.obtener_respuesta()
        self.assertEqual("ok", respuesta["status"])
        self.assertEqual([{'lote': 1, 'monto': 500, 'licitante': 1}, {'lote': 1, 'monto': 600, 'licitante': 1}],
                         respuesta["items"])
