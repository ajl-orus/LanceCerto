...
# db = CreadorDeBasesDeDatosMySql(["localhost", "root", "gTp8xT2!", "BD_LanceCerto"]).construir()
...
def registrar(nombre: str = Form(), apellido: str = Form(), email: str = Form(), usuario: str = Form(), clave: str = Form(),
    ...
    controlador.agregar(nombre, apellido, email, usuario, clave, nacimiento, TipoDeUsuario.Licitante)
    return __cambiar_status_code(controlador.obtener_respuesta(), response)
...
def pujar(lote_uid: int = Form(), licitante_uid: int = Form(), monto: int = Form(), response: Response = Response()):
    controlador = ControladorPuja(db)
    controlador.agregar(lote_uid, licitante_uid, monto)
    return __cambiar_status_code(controlador.obtener_respuesta(), response)
