...
from model.usuarios import Consignatario, Martillero, Licitante, Usuarios, Usuario
...
class UsuariosEnMemoria(Usuarios):
    ...
    def buscar(self, usuario: str, clave: str) -> Usuario:
        if usuario in self.__usuarios:
            ...
            if registro["clave"] == clave:
                return Usuarios.crear(registro["id"], registro["nome"], registro["sobrenome"], registro["email"],
                                      registro["usuario"], registro["clave"], registro["nascimento"], registro["tipo"])
    ...
    def buscar_licitante_por_uid(self, uid: int) -> Licitante:
        return self._buscar_por_uid_y_tipo(uid, TipoDeUsuario.Licitante)
    ...
    def actualizar(self, cuenta: Usuario, usuario: str, email: str, clave: str) -> None:
        ...
        self.__usuarios[usuario] = {
            ...
            "nome": cuenta.obtener_nombre(),
            "sobrenome": cuenta.obtener_apellido(),
            "usuario": usuario,
            "clave": clave,
            "email": email,
            "nascimento": cuenta.obtener_nacimiento(),
...
class PujasEnMemoria(Pujas):
    ...
    def agregar(self, monto: int, licitante: Licitante, lote: Lote):
        self.__pujas.append(Puja(self.__next_id, monto, licitante, lote))
        self.__next_id += 1
...
class LibroDiarioEnMemoria(LibroDiario):
    ...
    def listar_compras_de(self, licitante: Licitante):
        return [venta for venta in self.__ventas if venta.obtener_ganador().obtener_uid() == licitante.obtener_uid()]
