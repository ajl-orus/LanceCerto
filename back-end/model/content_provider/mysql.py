...
from model.usuarios import Consignatario, Martillero, Licitante, Usuario, Usuarios
...
class MysqlDatabase:
    def __init__(self, connection_string: list[str]):
        ...
        self.__default = ["localhost", "root", "1234", "BD_LanceCerto"]
        ...
        if not self.__conexion:
            try:
                ...
                if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                    raise ValueError("Erro de usuario ou senha")
                    ...
                    cursor.execute("CREATE TABLE IF NOT EXISTS Pujas ("
                                   "id int not null unique auto_increment primary key,"
                                   "monto int not null,"
                                   "id_licitante int not null,"
                                   "id_lote int not null,"
                                   "constraint fk_id_licitante foreign key (id_licitante) references Usuarios(id),"
...
class TablaUsuarios(Usuarios):
    ...
    def buscar_licitante_por_uid(self, uid: int) -> Licitante:
        return self.__db.obtener_uno(self.BUSCAR_USUARIO_POR_ID_Y_TIPO, (uid, TipoDeUsuario.Licitante.value),
                                     lambda r: Usuarios.crear(r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7]))
...
class TablaPujas(Pujas):
    CREAR_PUJA = "INSERT INTO Pujas(id_licitante, id_lote, monto) VALUES (%s,%s,%s)"
    BUSCAR_PUJA = "SELECT id, id_licitante, id_lote, monto FROM Pujas WHERE id = %s"
    BUSCAR_POR_LOTE = """
        SELECT Pujas.id, id_licitante, nombre, apellido, email, usuario, clave, nacimiento, tipo_usuario, monto
          FROM Pujas, Usuarios
         WHERE id_lote = %s
           AND id_licitante = Usuarios.id
    ...
    def agregar(self, licitante: Licitante, lote: Lote, monto: int):
        self.__db.insertar(self.CREAR_PUJA, (licitante.obtener_uid(), lote.obtener_uid(), monto))
    ...
    def buscar_por_uid(self, uid: int) -> Puja:
        return self.__db.obtener_uno(self.BUSCAR_PUJA, (uid,), lambda r: Puja(r[0], r[1], Licitante(r[2]), Lote(r[3])))
...
class TablaVentas(LibroDiario):
    ...
    BUSCAR_VENTA_CON_DATOS = """
            SELECT v.id, p.id, p.id_licitante, u.nombre, u.apellido, u.email, u.usuario, u.clave,
                   u.nacimiento, u.tipo_usuario, v.precio_final, v.comision, v.pago_consignatario
              FROM Ventas v
        INNER JOIN Pujas p on p.id = id_puja
        INNER JOIN Usuarios u on u.id = p.id_licitante
             WHERE v.id = %s"""
    LISTAR_COMPRAS = """
            SELECT v.id, p.id, p.id_licitante, u.nombre, u.apellido, u.email, u.usuario, u.clave,
                   u.nacimiento, u.tipo_usuario, v.precio_final, v.comision, v.pago_consignatario
              FROM Usuarios u
        INNER JOIN Pujas p ON u.id = p.id_licitante
    ...
    def listar_compras_de(self, licitante: Licitante) -> list[Venta]:
        return self.__db.obtener_muchos(self.LISTAR_COMPRAS, (licitante.obtener_uid(),),
